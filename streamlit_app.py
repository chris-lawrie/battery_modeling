...
with header:
    ...
    st.markdown("This model considers a typical day for a battery storage system co-located with solar panels. The price of electricty changes throughout the day, as does the avaliable solar. Given these variations, what is the most money we can make?")
...
with col2:
    ...
    battery_cap = st.slider("Battery Capacity (MW DC)", 5, 25, value = 10, step = 5)
    energy_cap  = st.slider("Battery Energy Capicty (MWh)", 10, 100, value = 40, step = 10)
    grid_cap    = st.slider("Grid Connection Capacity (AC MW)", 10, 100, value = 40, step = 10)      # DC-AC inverter and grid connection capacity (rated in AC MW)
...
with results:
    if st.button('Solve Model'):
        df_ans, tot, solve_time = solve_model(df,
                                solar_cap,
                                battery_cap,
                                energy_cap,
                                grid_cap,
